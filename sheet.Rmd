...
output: 
    pdf_document:
        extra_dependencies: makecell,wasysym,multicol,booktabs
...
# char = import5eChar::importCharacter(file = PUTCHARFILEHERE)
char = import5eChar::importCharacter(file = "C:/Users/Ogan/Downloads/Neferma'at_Cleric6")
...
if(!is.null(char$spells)){
    ...
    for(i in seq_along(availableLevels)){
        if (availableLevels[i] ==0){
            ...
            text = paste('Level',availableLevels[i],'(',char$spellSlots[i+1],')')
        }
        out = out %>% group_rows(text, groups[i],groups[i+1]-1, latex_gap_space = "1em")
    }
    out
}
